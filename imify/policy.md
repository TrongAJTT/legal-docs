...
- **Local Image & AI Processing:** All image processing, format conversion, compression, and AI-powered background removal are executed strictly **client-side** (locally on your device) utilizing WebAssembly (WASM), WebGPU, and Web Workers. Your files, images, and model outputs never leave your browser and are never uploaded to any external servers.
- **Local QR Scanning:** Scanning QR codes via camera or screen capture is processed entirely in your local browser memory using client-side JavaScript. No video stream, screen recording, or captured image is ever transmitted or stored outside your device.

**3. Local Storage and Preferences**

To enhance your experience, Imify saves your tool presets, UI preferences, and workspace configurations locally on your device using LocalStorage or IndexedDB. AI models downloaded for background removal are cached locally in your browser's Cache Storage or Origin Private File System (OPFS) for offline capability. This data is used solely to maintain your preferred settings and enable offline usage across sessions and is never transmitted to our servers or any third parties.
...
- **Camera & Screen Capture:** The QR Reader requires temporary permission to access your device's camera or capture active screen regions. These streams are used strictly for real-time decoding on your device and are immediately discarded.
